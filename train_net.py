    ...
    Args:
        cfg: an object with the following attributes:
            ...
            train: other misc config defined in `configs/common/train.py`, including:
                ...
                ddp (dict)
    """
    to_save_dirs = ["data", "zsd_detr"]
    utils.save_custom_modules(to_save_dirs, cfg.train.output_dir)
    ...
    trainer.register_hooks(
        [
            ...
            hooks.BestCheckpointer(
                eval_period=cfg.train.checkpointer.period,
                checkpointer=checkpointer,
                val_metric="bbox/unseen_AP50",
            )
            if comm.is_main_process()
            else None,
        ]
...
def main(args):
    ...
    if cfg.train.output_dir.endswith("_v"):
        cfg.train.output_dir = utils.get_output_dir_from_tempelate(cfg.train.output_dir)
...
if __name__ == "__main__":
    ...
    if args.eval_only:
        _eval_output_dir = "./output/inference"
        shutil.rmtree(_eval_output_dir, ignore_errors=True)
        os.makedirs(_eval_output_dir, exist_ok=True)
        args.opts.extend([f"train.output_dir={_eval_output_dir}", f"dataloader.evaluator.output_dir={_eval_output_dir}"])
