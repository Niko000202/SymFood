...
class ZSDDETRTransformerEncoder(TransformerLayerSequence):
    def __init__(
        self,
        embed_dim: int = 256,
        num_heads: int = 8,
        ...
        super(ZSDDETRTransformerEncoder, self).__init__(
...
class ZSDBaseTransformerLayer(nn.Module):
    def __init__(
        self,
        attn: List[nn.Module],
        ffn: nn.Module,
        ...
        super(ZSDBaseTransformerLayer, self).__init__()
        assert set(operation_order).issubset({"self_attn", "norm", "cross_attn", "cross_attn2", "ffn"})

        # count attention nums
        num_attn = (
            operation_order.count("self_attn") + operation_order.count("cross_attn") + operation_order.count("cross_attn2")
        )
        ...
        for operation_name in operation_order:
            if operation_name in ["self_attn", "cross_attn", "cross_attn2"]:
    ...
    def forward(
        ...
        **kwargs,
        ...
        for layer in self.operation_order:
            if layer == "self_attn":
            ...
            elif layer == "cross_attn2":
                word_vec = self.word_vec.unsqueeze(dim=0).expand(query.shape[0], -1, -1)
                word_vec_padding_mask = torch.zeros(*word_vec.shape[:2], dtype=torch.bool, device=word_vec.device)
                if self.training:
                    word_vec = word_vec[torch.randperm(word_vec.shape[0])]
                ...
                query = self.attentions[attn_index](
                    ...
                    key_pos=None,
                    attn_mask=attn_masks[attn_index],
                    key_padding_mask=word_vec_padding_mask,
                    **kwargs,
                )
...
class ZSDDETRTransformerDecoder(TransformerLayerSequence):
    def __init__(
        self,
        embed_dim: int = 256,
        num_heads: int = 8,
        ...
        operation_order = ("self_attn", "norm", "cross_attn", "norm", "cross_attn2", "norm", "ffn", "norm")

        super(ZSDDETRTransformerDecoder, self).__init__(
            transformer_layers=ZSDBaseTransformerLayer(
                attn=attn,
                ffn=FFN(
                    embed_dim=embed_dim,
                    feedforward_dim=feedforward_dim,
    ...
    def forward(
        ...
        **kwargs,
        ...
        word_vec = self.word_vec_proj(self.word_vec)
...
class ZSDDETRTransformer(nn.Module):
    def __init__(
        self,
        norm_features: bool,
        encoder=None,
    ...
    def forward(
        ...
        **kwargs,
        ...
        enc_outputs_embedding = self.decoder.class_embed[self.decoder.num_layers](output_memory)
        if self.norm_features:
            enc_outputs_embedding = enc_outputs_embedding / enc_outputs_embedding.norm(dim=-1, keepdim=True)
        enc_outputs_class = enc_outputs_embedding @ self.word_vec.t()
